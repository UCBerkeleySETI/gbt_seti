function [OutImageFileCell,OutputMatrixCell]=imifft_fits(ImInput,ImOutput,Type,varargin);
%-----------------------------------------------------------------------------
% imifft_fits function                                                ImBasic
% Description: Calculate the 2D inverse Fast Fourier Transform of FITS images.
...
%            'Sym'      - {1 | 0}. 1 causes IFFT2 to treat F as
%                         conjugate symmetric in two dimensions so that
%                         the output is purely real. Default is0.
%            'Shift'    - {1 | 0}. 1 shift the zero frequency component
%                         to the center of the spectrum. Specifically,
...
% Example: Out=imifft_fits('lred0015.fits','try.fits');
% Reliable: 2
%-----------------------------------------------------------------------------
...
for Iim=1:1:Nim,
   ...
   switch InPar.Sym
    case 0
       InputImage = ifft2(InputImage);
    case 1
       InputImage = ifft2(InputImage,'symmetric');
    otherwise
       error('Unknwon Sym option');
   end
   ...
   [HeaderInfo] = cell_fitshead_addkey(HeaderInfo,...
                                       Inf,'COMMENT','','Created by imifft_fits.m written by Eran Ofek',...
                                       Inf,'HISTORY','',sprintf('iFFT output type: %s',Type),...
                                       Inf,'HISTORY','',sprintf('iFFT symmetric option: %d',InPar.Sym),...
				       Inf,'HISTORY','',sprintf('iFFT shift option: %d',InPar.Shift),...
