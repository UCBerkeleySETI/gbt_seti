...
% Example: [HTM,LevList]=htm_build(7);
%          I=htm_search_point(HTM,[1,1])
% Reliable: 2
%--------------------------------------------------------------------------
...
while Cont
    Ind = Ind + 1;
    Flag = in_polysphere(CD,HTM(Ind).cosd);
    Cont = ~Flag;
    if (Ind>8),
...
while Cont
    ...
    if (isempty(HTM(Ind).son)),
        ...
        while ~Flag
            I = I + 1;
            Flag = in_polysphere(CD,HTM(HTM(Ind).son(I)).cosd);
        end
        Ind = HTM(Ind).son(I);
