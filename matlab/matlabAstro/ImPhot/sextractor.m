...
%                       {'XWIN_IMAGE','YWIN_IMAGE','ALPHAWIN_J2000','DELTAWIN_J2000','MAG_AUTO','MAGERR_AUTO','MU_MAX','FLUX_MAX','FLUX_AUTO','FLUXERR_AUTO'}
...
% Example: [Cat,SexKeys,Col]=sextractor('P*.fits');
%          [Cat,SexKeys,Col]=sextractor('P*.fits',[],'OutType','sim');
%          [Cat,SexKeys,Col]=sextractor('*.fits','OutType','sim_t');
% Reliable: 2
%--------------------------------------------------------------------------
CurFunDirName = which_dir(mfilename);
...
DefV.InSim           = [];
DefV.SexKeys         = {'XWIN_IMAGE','YWIN_IMAGE','ALPHAWIN_J2000','DELTAWIN_J2000','MAG_AUTO','MAGERR_AUTO','MU_MAX','FLUX_MAX','FLUX_AUTO','FLUXERR_AUTO','MAG_APER','MAGERR_APER'};
...
DefV.Read2Mat        = true;
DefV.OutType         = 'struct';  % {'cell'|'struct'|'struct_t'|'sim'|'sim_t'|'table'}
DefV.CatName         = [];
%DefV.OutPrefix       = '';
%DefV.OutDir          = '';
DefV.SexPars         = {};
...
ProgFullPath   = construct_fullpath(InPar.ProgName,InPar.ProgPath,CurFunDirName);
ParamFullPath  = construct_fullpath(InPar.ParamPath,'.',CurFunDirName);
ConfigFullPath = construct_fullpath(InPar.ConfigPath,'.',CurFunDirName);

% convolution kernel
ConvFile = [];
if (ischar(InPar.ConvPath)),
    ConvFullPath   = construct_fullpath(InPar.ConvPath,'.',CurFunDirName);
elseif (isnumeric(InPar.ConvPath)),
    if (isempty(InPar.ConvPath)),
        ...
        Str    = str_duplicate('%11.7f ',SizeMF(2),'\n');
...
if (issim(Images) || isstruct(Images)),
    ...
    [~,ImList] = create_list(Images,NaN);
    TmpImages = false;
end

if (issim(DblImages) || isstruct(DblImages)),
    % write temp FITS images
    if (InPar.Verbose),
    ...
    % create list of FITS images
    [~,DblImList] = create_list(DblImages,NaN);
    Ndblim    = numel(DblImList);
    TmpDblImages = false;
end
...
ExtraParStr = construct_keyval_string(InPar.SexPars);
...
if (isempty(InPar.SexKeys)),
    ...
    if (ischar(InPar.SexKeys)),
        ...
        ParamFile      = create_list(InPar.SexKeys,[],'n');   % convert to file
        IsTmpParamFile = true;
    end
end
