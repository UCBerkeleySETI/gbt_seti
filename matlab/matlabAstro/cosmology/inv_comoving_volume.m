...
%---------------------------------------------------------------------------
% inv_comoving_volume function                                    cosmology
...
% Example: [~,V]=comoving_volume(2); [Z]=inv_comoving_volume(V)
% Reliable: 2
%---------------------------------------------------------------------------
Def.Type = 'LD';
Def.CosmoPars = 'wmap9';
...
Z       = zeros(Ndm,1);
for Idm=1:1:Ndm,
   Z(Idm) = fun_binsearch(@comoving_volume_handle,Volume(Idm),[1e-15 100],1e-3,CosmoPars);
end

function Vol=comoving_volume_handle(z,CosmoPars)
[~,Vol]=comoving_volume(z,CosmoPars);
