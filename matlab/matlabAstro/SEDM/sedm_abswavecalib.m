function [AbsCalib,SegmentsInfo]=sedm_abswavecalib(varargin)
%--------------------------------------------------------------------------
...
% smooth K(1), K(2) paramaeters
if (InPar.FitSurface)
    K12 = [AbsCalib.Arc_BestK];
    K1  = K12(1:2:end-1).';
    K2  = K12(2:2:end).';
    ...
    [K1Surface.Res,~,K1Surface.FunFun]=fit_2d_polysurface(Xa,Ya,K1,[],'X',InPar.PolyXoffset,...
                                                                    'Y',InPar.PolyYoffset,...
                                                                    'XY',InPar.PolyXYoffset);

    [K2Surface.Res,~,K2Surface.FunFun]=fit_2d_polysurface(Xa,Ya,K2,[],'X',InPar.PolyXoffset,...
                                                                    'Y',InPar.PolyYoffset,...
                                                                    'XY',InPar.PolyXYoffset);

    % apply the best fit surface to all spexcells
    PredictedK1 = K1Surface.FunFun(K1Surface.Res.Par,[SegmentsInfo.MeanX].',[SegmentsInfo.MeanY].');
    PredictedK2 = K1Surface.FunFun(K2Surface.Res.Par,[SegmentsInfo.MeanX].',[SegmentsInfo.MeanY].');

    PredK1Cell = num2cell(PredictedK1);
    [SegmentsInfo(1:1:Nseg).PredictedK1] = deal(PredK1Cell{:});

    PredK2Cell = num2cell(PredictedK2);
    [SegmentsInfo(1:1:Nseg).PredictedK2] = deal(PredK2Cell{:});
else
    
   error('Unknown FitSurface option'); 
end
% for all spexcells
% refit K1
...
for Iseg=1:1:Nseg,
    ...
    % re-fit K1 for all spexcells
    SpecPix = (1:1:length(SegmentsInfo(Iseg).(InPar.SpecField))).';
    SpecInt = SegmentsInfo(Iseg).(InPar.SpecField).';
       
    BestCorr = 1;
    for Ik1=1:1:length(InPar.FineVecK1),
        K1 = InPar.FineVecK1(Ik1);
        NegCorr=sedm_wavelength_xcorr([SegmentsInfo(Iseg).PredictedK1, SegmentsInfo(Iseg).PredictedK2],...
                                      [SpecPix, SpecInt],...
                                      [CalRefSpecPix, RefSpecInt],...
                                      K1,...
                                      InPar.FunDispLam,InPar.LowerK,InPar.UpperK);
    ...
    SpecWave = InPar.FunDispLam([SegmentsInfo(Iseg).PredictedK1, SegmentsInfo(Iseg).PredictedK2],...
                                SpecPix,...
