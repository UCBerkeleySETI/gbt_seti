...
for Ichild=1:1:Nchild,
   [X,N] = realhist(get(Hchild(Ichild),'XData'),[XLim InPar.NbinX]);
   HistX(Ichild).X = X;
   HistX(Ichild).N = N;
   [X,N] = realhist(get(Hchild(Ichild),'YData'),[YLim InPar.NbinX]);
   HistY(Ichild).X = X;
   HistY(Ichild).N = N;
...
for Ichild=1:1:Nchild,
   H=hist_stairs(HistX(Ichild).X,HistX(Ichild).N,'k-','Type','v');
...
for Ichild=1:1:Nchild,
   H=hist_stairs(HistY(Ichild).X,HistY(Ichild).N,'k-','Type','h');
   set(H,'Color',Color(Ichild).Color);
   switch lower(InPar.Scale)
    case 'log'
       set(Hy,'XScale','log');
