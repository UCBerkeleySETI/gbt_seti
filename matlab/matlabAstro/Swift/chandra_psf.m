function [PSF,PSFxVec,PSFyVec]=chandra_psf(Energy,DetX,DetY,varargin)
%--------------------------------------------------------------------------
% chandra_psf function                                               Swift
...
% Example: [PSF,X,Y]=chandra_psf(1,4090,4090,'FileName',1);
% Reliable: 2
%--------------------------------------------------------------------------
...
if (~isempty(InPar.SaveFITS)),
    % save FITS
    HeaderInfo = cell_fitshead_addkey([],'COMMENT','','Chandra PSF created by chandra_psf.m');
    HeaderInfo = cell_fitshead_addkey(HeaderInfo,'Camera',InPar.Camera,'Chandra camera');
    HeaderInfo = cell_fitshead_addkey(HeaderInfo,'Interp',InPar.InterpMethod,'PSF interpolation method');
    HeaderInfo = cell_fitshead_addkey(HeaderInfo,'CalDB',InPar.CalDBdir,'CalDB dir');
    HeaderInfo = cell_fitshead_addkey(HeaderInfo,'File',InPar.FileName,'Original PSF file');

    [Flag,HeaderInfo] = fitswrite(PSF,InPar.SaveFITS,HeaderInfo);
end
