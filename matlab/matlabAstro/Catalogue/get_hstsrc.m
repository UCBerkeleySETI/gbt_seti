...
AllCat  = zeros(0,13);
for Icat=1:1:Ncat,
    Cat = load2(sprintf('%s%s',PathCat,sprintf(CatFileName,HSTsrc.Cat(Res.IndCat(Icat),HSTsrc.Col.ImageID) )));
    AllCat = [AllCat;Cat.Cat];
end
D = sphere_dist(AllCat(:,Cat.Col.RA),AllCat(:,Cat.Col.Dec),RA,Dec);
Cat.Cat = AllCat(find(D<Radius),:);
