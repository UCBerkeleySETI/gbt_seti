...
%--------------------------------------------------------------------------
% wget_sdss function                                             Catalogue
% Description: Query SDSS PhotoPrimary table around specific coordinate.
%              See run_sdss_sql.m for a more general queries.
...
% Example: [Cat,ColCell,Col]=wget_sdss('12:00:00','+30:00:00',0.1./RAD);
% Reliable: 2
%--------------------------------------------------------------------------
RAD    = 180./pi;
InvRAD = pi./180;
...
MinRA  = (RA  - SearchRad(1)./cos(MaxDec)).*RAD;
MaxRA  = (RA  + SearchRad(1)./cos(MaxDec)).*RAD;
...
switch lower(Shape)
    case {'circle','circ'}
        if (~isempty(Cat)),
            D = sphere_dist_fast(Cat(:,Col.ra).*InvRAD,Cat(:,Col.dec).*InvRAD,RA,Dec);
