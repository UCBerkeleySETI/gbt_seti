...
%            'ColX'    - Column index of the longitude in the input
%                        catalog. Default is 1.
%            'ColY'    - Column index of the latitude in the input
...
CatUM = true(Ncat,1);
Res   = struct('IndCat',cell(1,Nin),'DistRAD',cell(1,Nin),'PA',cell(1,Nin),'Nfound',num2cell(zeros(1,Nin)));
if (~isempty(Cat)),
    ...
    switch lower(InPar.SearchMethod)
        ...
        case 'binm'
            ...
            Ind2 = mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'+InPar.SearchRad)+1;
       ...
       case 'binms'
            switch lower(InPar.CooType),
                case 'sphere'
                    ...
                    Ind1 = mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'-InPar.SearchRad);
                    Ind2 = mfind_bin(Cat(:,InPar.SortCol),RefCat(:,InPar.RefSearchCol).'+InPar.SearchRad);
