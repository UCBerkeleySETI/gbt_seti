...
%            See convert_flux.m to transform from general units to F_lambda.
...
% Example: Kcorr=kcorr(Spec,2,'GALEX','NUV','AB',1,'2MASS','J','Vega')
% Reliable: 2
%--------------------------------------------------------------------------

SpecZ1 = shift_spec(Spec,Z1,'w','f');   % Flux should be corrected by (1+z)
SpecZ2 = shift_spec(Spec,Z2,'w','f');   % Flux should be corrected by (1+z)

Mag1 = synphot(SpecZ1,FiltFam1,FiltName1,MagSys1);
Mag2 = synphot(SpecZ2,FiltFam2,FiltName2,MagSys2);
Kcorr = Mag1 - Mag2;
