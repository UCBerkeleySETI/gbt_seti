...
%          - String of filter family (see get_filter.m for options).
%            e.g., 'PTF', 'SDSS'
...
for Ilc=1:1:Nlc,
    ...
    if (ischar(LC{Ilc})),
        ...
        LCdata{Ilc} = read_mark_lc(LC{Ilc});
    ...
    [SynPhot(Ilc), FlagSynPhot(Ilc),FilterWave(Ilc)] = synphot(Spec,FiltFam{Ilc},FiltName{Ilc},FiltSys{Ilc});
end
...
GalExtinCorr = 10.^(+0.4.*(extinction(InPar.Ebv,ObsRestSpec(:,1)./10000,[],InPar.Rv)));
...
RestExtinCorr = 10.^(+0.4.*(extinction(InPar.Ebv_z,CorrSpec(:,1)./10000,[],InPar.Rv_z)));
