...
% Reliable: 2
%------------------------------------------------------------------------------
Dir = which_dir('SpecPhot_Stand.mat');
PWD = pwd;
IndExist = [];
...
if (isempty(Dir)),
   ...
   SpecPhot_Stand = load2('SpecPhot_Stand.mat');
...
SpecPhot_Stand(I).RA       = convertdms(RA,'gH','r');
SpecPhot_Stand(I).Dec      = convertdms(Dec,'gD','r');
...
if (strcmpi(R,'y')),
    ...
    FlagT = find_ranges_flag(SpecPhot_Stand(I).Spec(:,1),Telluric.Cat(:,1:2));
