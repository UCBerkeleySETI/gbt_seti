...
% Example: [SN,OP,S]=sn_calc('ExpTime',1./30,'SN',10); SN.Mag, OP.PixScale, OP.totFOVarea, OP.totFOVarea.*S.StarDensity
%          [SN,OP]=sn_calc('Mag',[16;17]);
%          [SN,OP]=sn_calc('SN',[5;10]);
% Reliable: 2
%--------------------------------------------------------------------------
...
%--- Telescope ----
%DefV.Tel          = [];
DefV.Aper         = 55; %1.8; %55;        % [cm] Telescope aperture
DefV.FocalLength  = 108; %1.7; %108;        % [cm] Telescope focal length
DefV.Family       = 'Cousins'; %'Cousins'; %'Cousins'; %'Johnson'; %'SDSS';  %
DefV.Band         = 'R'; %'R'; %'R'; %'r';        %
DefV.MagType      = 'AB'; %'Vega'; %'AB';       % {'AB','Vega'}
%--- Optics ---
DefV.QOE          = 0.45; %[0.6 0.75 0.85];        % []   quantum-optical effiency [including CCD, optics and filter]
DefV.FWHM         = 2.5; %3.7;          % ["]  Point source image FWHM (FWHM=2.3548\sigma) [including pixel size, optics, diffraction, jitter, charge leakage]
%--- CCD ---
DefV.Bin          = 1;           % [pix]     pixel binning
DefV.PixSize      = 6.5;         % [micron]  CCD pixel size
DefV.SizeCCD      = [2048 2500]; % [pix pix] size of a single CCD
...
%--- Exposure ---
DefV.ExpTime      = 1./30;           % expoure time per a single CRsplit
...
%--- Background ---
DefV.Back         = 19.9; %26.78; %19.9; %20.3;  % 21.3 % mag/arcsec^2 in filter
...
%--- Photometry ---
DefV.PhotRad      = (1.5:0.5:100);  % ["] radius of photometry aperture
...
if (ischar(InPar.Family)),
    Filter = get_filter(InPar.Family,InPar.Band);
    % normalize band maximum to 1
else
    Filter.nT{1} = InPar.Family;
end
% normalize band maximum to 1
Filter.nT{1}(:,2) = Filter.nT{1}(:,2)./max(Filter.nT{1}(:,2));
...
%------------------------------
if (~isempty(InPar.AM)),
    [~,ExtFactor]=atmospheric_ext(Filter.nT{1},InPar.AM);
    % calculate the weighted mean of extinction factor within filter
    % assuming flat f_lambda spectrum
    OutPars.ExtinFactor = sum(ExtFactor.*Filter.nT{1}(:,2))./sum(Filter.nT{1}(:,2));
else
...
%  
   ...
   for Imn=1:1:Net,
      [Flux(Imn),Counts(Imn)] = spec_photon_counts(InPar.Temp(Imn),Filter.nT{1},[],SolR,10);
   end
   
   % for Mag 0 star
   DeltaMag0  = blackbody_mag_c(InPar.Temp,InPar.Family,InPar.Band,InPar.MagType,SolR,10);
   Flux0      = Flux.*10.^(+0.4.*DeltaMag0);
   Counts0    = Counts.*10.^(+0.4.*DeltaMag0);
   
   
   DeltaMag  = blackbody_mag_c(InPar.Temp,InPar.Family,InPar.Band,InPar.MagType,SolR,10) - Mag;
