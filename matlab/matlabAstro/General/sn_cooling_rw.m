...
%--------------------------------------------------------------------------
% sn_cooling_rw function                                           General
...
% Example: [L,Tc,R,Time,Mag]=sn_cooling_rw;
% Reliable: 2
%--------------------------------------------------------------------------
SolRad = 696000e5;  % [cm]
...
switch lower(InPar.Type)
    ...
    case 'wr'
        ...
        if (InPar.Z==0),
            ...
            Tc  = convert_energy(1.33,'eV','T').* InPar.f_rho.^-0.02 .* R12.^0.20.* t5.^-0.38;   % T_ph >= 1.07
            t5_b = (1.07./(1.33.*InPar.f_rho.^-0.02.*R12.^0.20)).^(-1./0.38);
            I   = find(Tc<convert_energy(1.07,'eV','T'));
            %t5(I(1))
            Tc(I) = convert_energy(1.07,'eV','T').* (t5(I)./t5_b).^-0.12;   % T_ph <  1.07
            ...
            kappa_He    = 0.085 .* (1-InPar.Z).*(Tc./convert_energy(1.07,'eV','T')).^0.88;   % T>= 1.07 eV
            kappa_He(I) = 0.085 .* (1-InPar.Z).*(Tc(I)./convert_energy(1.07,'eV','T')).^10;   % T<  1.07 eV
   
            kappa_Z     = 0.043.*InPar.Z.*(Tc./convert_energy(1.0,'eV','T')).^1.27;

            Ikappa = find(kappa_Z>kappa_He);

            Tc(Ikappa) = convert_energy(1.5,'eV','T') .* InPar.f_rho.^-0.017 .* InPar.Z.^-0.2 .*...
...
if (nargout>3),
    if (isempty(InPar.Dist)),
        Dist = lum_dist(InPar.redshift);
    else
        Dist = InPar.Dist.*1e6;  % [pc]
        InPar.redshift = inv_lum_dist(Dist);
    end
    ...
    if (isempty(InPar.Wave)),
        ...
        Mag = blackbody_mag_c(Tc./(1+InPar.redshift),InPar.FiltFam,InPar.FiltName,InPar.FiltSys,R,Dist);
    else
        Pc = get_constant('pc');
        [~,In]=black_body(Tc./(1+InPar.redshift),InPar.Wave);
        Mag = convert_flux(In.*R.^2./(Dist.*Pc).^2,'cgs/Hz','AB',InPar.Wave,'A');
    end
    if (InPar.Ebv>0),
        A = extinnction(InPar.Ebv,InPar.FiltFam,InPar.FiltName,InPar.Rv);
        Mag = Mag + A;
    end
end
