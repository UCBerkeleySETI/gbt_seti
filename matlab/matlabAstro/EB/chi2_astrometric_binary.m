...
%          [Chi2,Res]=chi2_astrometric_binary([365,0.5,1],Orbit.JD,Orbit.X,Orbit.Y,[],[]);
% Reliable: 2
%--------------------------------------------------------------------------
...
if (e<1 && e>0),
    ...
    M = n.*(t-T);
    a = 1;  % scale a
    [Nu,R] = kepler_elliptic(M,a.*(1-e),e,NaN);
    ...
    Res.El = thiele_innes2el(Res.Par(1),Res.Par(2),Res.Par(3),Res.Par(4));
else
