...
SizeDate = size(Date);
if (SizeDate(2)==3),
   JD = julday([Date, zeros(SizeDate(1))]);
else
   JD = Date;
end
...
[SunRA_p0, SunDec_p0] = suncoo(JD,'a');
[SunRA_m2, SunDec_m2] = suncoo(JD-2,'a');
[SunRA_m1, SunDec_m1] = suncoo(JD-1,'a');
[SunRA_p1, SunDec_p1] = suncoo(JD+1,'a');
[SunRA_p2, SunDec_p2] = suncoo(JD+2,'a');
...
for I=1:1:N,
   ...
   %--- Rise/Set
   [T_RS,Alt_RS,Az_RS] = rise_set([SunRA, SunDec], GAST(I) ,RS_Alt,  ObsCoo, DeltaT);
   T_RS = T_RS + TimeZone./24;
   if (T_RS(2)>1),
      [T_RSp,Alt_RSp,Az_RSp] = rise_set([SunRA_p, SunDec_p], GASTp(I) ,RS_Alt,  ObsCoo, DeltaT);
      T_RS(2)  = T_RSp(2);
      Az_RS(2) = Az_RSp(2);
   end
   if (T_RS(1)<0),
      [T_RSm,Alt_RSm,Az_RSm] = rise_set([SunRA_m, SunDec_m], GASTm(I) ,RS_Alt,  ObsCoo, DeltaT);
      T_RS(1) = T_RSm(1);
      Az_RS(1) = Az_RSm(1);
   end
   ...
   %--- Civil Twilight
   [T_06,Alt_06,Az_06] = rise_set([SunRA, SunDec], GAST(I) ,T06_Alt, ObsCoo, DeltaT);
   T_06 = T_06 + TimeZone./24;
   if (T_06(2)>1),
      [T_06p,Alt_06p,Az_06p] = rise_set([SunRA_p, SunDec_p], GASTp(I) ,T06_Alt,  ObsCoo, DeltaT);
      T_06(2) = T_06p(2);
   end
   if (T_06(1)<0),
      [T_06m,Alt_06m,Az_06m] = rise_set([SunRA_m, SunDec_m], GASTm(I) ,T06_Alt,  ObsCoo, DeltaT);
      T_06(1) = T_06m(1);
   end


   %--- Nautical Twilight
   [T_12,Alt_12,Az_12] = rise_set([SunRA, SunDec], GAST(I) ,T12_Alt, ObsCoo, DeltaT);
   T_12 = T_12 + TimeZone./24;
   if (T_12(2)>1),
      [T_12p,Alt_12p,Az_12p] = rise_set([SunRA_p, SunDec_p], GASTp(I) ,T12_Alt,  ObsCoo, DeltaT);
      T_12(2) = T_12p(2);
   end
   if (T_12(1)<0),
      [T_12m,Alt_12m,Az_12m] = rise_set([SunRA_m, SunDec_m], GASTm(I) ,T12_Alt,  ObsCoo, DeltaT);
      T_12(1) = T_12m(1);
   end
   ...
   [T_18,Alt_18,Az_18] = rise_set([SunRA, SunDec], GAST(I) ,T18_Alt, ObsCoo, DeltaT);
   T_18 = T_18 + TimeZone./24;
   if (T_18(2)>1),
      [T_18p,Alt_18p,Az_18p] = rise_set([SunRA_p, SunDec_p], GASTp(I) ,T18_Alt,  ObsCoo, DeltaT);
      T_18(2) = T_18p(2);
   end
   if (T_18(1)<0),
      [T_18m,Alt_18m,Az_18m] = rise_set([SunRA_m, SunDec_m], GASTm(I) ,T18_Alt,  ObsCoo, DeltaT);
      T_18(1) = T_18m(1);
   end
