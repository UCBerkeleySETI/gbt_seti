function U2=abberation(U1,V,Units);
%------------------------------------------------------------------------------
% abberation function                                                    ephem
% Description: Calculate the position of a star corrected for abberation
...
% Example: U2=abberation([convertdms([2 44 12.9747],'H','r'), convertdms([1 49 13 39.896],'D','R')],julday([13.19 11 2028]));
% Reliable: 2
%------------------------------------------------------------------------------
...
switch lower(Units)
 case 'agd'
    C = get_constant('c','cgs').*convert_units('cm/s','au/day');
 otherwise
    C = get_constant('c',Units);
end
...
if (size(V,2)==1),
   ...
   % expressions
   JD = V;
   V = earth_vel_ron_vondrak(JD,Units);
