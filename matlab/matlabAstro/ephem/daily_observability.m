...
%------------------------------------------------------------------------------
% daily_observability function                                           ephem
...
% See also: yearly_observability.m, obspl.m
% Tested : Matlab 5.3
%     By : Eran O. Ofek                    Aug 2001
%    URL : http://weizmann.ac.il/home/eofek/matlab/
% Example: daily_observability('Wise',[27 8 2001],[18 0 0],[1 67 0 0]);
% Reliable: 2
%------------------------------------------------------------------------------
RAD      = 180./pi;
...
if (SizeDate(2)==1),
   ...
   JDmid = julday([Date, 1-ObsLon(:,1)./(2.*pi)]);
end
...
elseif (nargin==5),
   ...
   if (Equinox~=2000),
      ...
      NewCoo = coco([RA, Dec],EqStr,'j2000.0','r','r');
      RA     = NewCoo(:,1);
      Dec    = NewCoo(:,2);
   else
      % already in J2000.0
...
[SunRA, SunDec]   = suncoo(JD,'j');
SunHC             = horiz_coo([SunRA, SunDec],JD,[ObsLon, ObsLat],'h');
SunAz             = SunHC(:,1);
SunAlt            = SunHC(:,2);

% moon coordinates during 24 hours.
[MoonRA, MoonDec] = mooncool(JD, [ObsLon, ObsLat]);
MoonHC            = horiz_coo([MoonRA, MoonDec],JD,[ObsLon, ObsLat],'h');
MoonAz            = MoonHC(:,1);
MoonAlt           = MoonHC(:,2);

% Object coordinates during 24 hours
ObjHC             = horiz_coo([RA, Dec],JD,[ObsLon, ObsLat],'h'); 
ObjAz             = ObjHC(:,1);
ObjAlt            = ObjHC(:,2);
ObjAirMass        = hardie([pi./2-ObjAlt]);
I = find(ObjAlt<3./RAD);
ObjAirMass(I) = NaN;
...
C_Ext      = 0.3;
Vsky       = 21.7;
[DelV, MoonElon, ObjMoonDist, MoonIllFrac] = moon_sky_brightness(JD, [RA, Dec], [ObsLon, ObsLat, 0], C_Ext, Vsky);
