...
if (isempty(Date)==1),
   ...
   T.Min    = Time(5);
   T.Sec    = Time(6);
   T.JD     = julday([T.Day, T.Month, T.Year, T.Hour, T.Min, T.Sec]);
...
T.Frac = convertdms([T.Hour, T.Min, T.Sec],'H','f');

T.LST  = lst(T.JD + UT1mUTC./86400,GeodLong,'m');
