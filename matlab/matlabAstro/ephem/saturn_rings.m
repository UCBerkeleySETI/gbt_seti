...
% Example: [SLO,SLS,DU,P,Major,Minor]=saturn_rings(2451545);
% Web example: http://astroclub.tau.ac.il/ephem/Saturn/index.php
% Reliable: 2
%---------------------------------------------------------------------------
RAD = 180./pi;
if (nargin==1),
   [Coo,Dist]=planet_ephem(JD,'Saturn','Earth','ecliptic');
   R     = Dist(:,2);
   Delta = Dist(:,1);
   L     = Coo(:,1);
   B     = Coo(:,2);
   [Coo,Vel] = calc_vsop87(JD,'Saturn','d','d');
   Ls    = Coo(1,:).';
   Bs    = Coo(2,:).';
else
