function [Links,FitsName,Images,Headers,CatCol]=get_sdss_imcat(ID,Save,DataRelease);
%------------------------------------------------------------------------------
% get_sdss_imcat function                                                 sdss
% Description: Given an SDSS field ID [Run, Rerun, Camcol, Field], get the
%              link to, and the SDSS object catalog (tsObj) associated
%              with a corrected image in FITS format.
...
% Example: [Link,FN,Image,Header,CatCol]=get_sdss_imcat([1339 40 1 11],'y');
% Description: http://www.sdss.org/dr7.1/dm/flatFiles/tsObj.html
% Reliable: 2
%------------------------------------------------------------------------------
RAD = 180./pi;
...
for Iim=1:1:Nim,
   ...
   ImageName = sprintf('tsObj-%06d-%d-%d-%04d.fit',Run,CamCol,ReRun,Field);
