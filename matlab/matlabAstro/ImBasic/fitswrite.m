function [Flag,HeaderInfo]=fitswrite(Image,FileName,HeaderInfo,DataType,varargin)
%--------------------------------------------------------------------------
% fitswrite function                                               ImBasic
% Description: Write a simple 2D FITS image.
%              USE fitswrite_my.m instead!
...
% Examples: [Flag,HeaderInfo]=fitswrite(rand(2048,1024),'Example.fits');
% Reliable: 2
%--------------------------------------------------------------------------
...
for Iarg=1:2:Narg-1,
   switch lower(varargin{Iarg})
    case 'identifyint'
       IdentifyInt     = varargin{Iarg+1};
    case 'resetdefkey'
       ResetDefKey     = varargin{Iarg+1};
    case 'overwrite'
       OverWrite       = varargin{Iarg+1};   
    otherwise
       error('Unknwon keyword option');
   end
end
...
switch lower(ResetDefKey)
   ...
   case 'y'
      ...
      HeaderInfo = cell_fitshead_addkey(HeaderInfo,...
                                        ...
                                        0,'BZERO' ,int32(0),'offset data range to that of unsigned short',...
                                        0,'BSCALE',int32(1),'default scaling factor');
   otherwise
