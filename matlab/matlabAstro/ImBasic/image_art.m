...
%                       Default is 'gauss'.
%            'ParPSF' - PSF parameters. Default is [2 2 0]
...
DefV.Back           = 300;       % e-
DefV.PSF            = 'gauss';
DefV.ParPSF         = [2 2 0];   % sigmaX, sigmaY Rho for 'gauss'
DefV.MaxRad         = 10;
...
switch lower(InPar.PSF)
    case 'gauss'
        for Istar=1:1:Nstar,
            ...
            OutImage = OutImage + bivar_gauss(MatX,MatY,...
...
for Inr=1:1:InPar.Nnoisereal,
    ...
    if (~isempty(InPar.OutFileName)),
        ...
        fitswrite_my(OutImageN,ImageName,InPar.Header,InPar.DataType);
    else
        ImageName = [];
    end
    if (InPar.OutSIM),
...
if (InPar.Nnoisereal==0),
    if (~isempty(InPar.OutFileName)),
        ...
        fitswrite_my(OutImage,ImageName,InPar.Header,InPar.DataType);
    else
        ImageName = [];
    end
    if (InPar.OutSIM),
